...
<div class="sans shop trailingSpace">
    <header class="navigation navigationWithToolbar">
        ...
        <nav class="toolbar trisected clearfix"><a href="{{route('favoriteIndex')}}" class="toolDrawer toolDrawerOpen">我收藏的</a><a href="{{route('myBuyedShop')}}" class="toolDrawer ">我买过的</a><a href="{{route('myBrowseShop')}}" class="toolDrawer">最近浏览</a></nav>

    </header>
    <section class="shopList">
        {{--<h2 class="shopListClassifier">今天</h2>--}}
        @foreach($favorites as $f)
        <div style="background-image:url('{{\Dajiayao\Library\Util\ImageUtil::getRuleImgSize($f->shop->banner,750,246)}}')" class="shopBanner">
            <div class="shopBio">
                <a href="{{Config::get('app.shop_base_url').$f->shop->short_id}}" class="shopBrand" style="background-image:url('{{$f->shop->thumbnail}}')"></a>
                <a href="{{Config::get('app.shop_base_url').$f->shop->short_id}}" class="shopName">{{$f->shop->name}}</a>
                <div class="shopFavor" data-shopshortid="{{$f->shop->short_id}}"><span class="yyicon-favorite"></span></div>
            </div>
        </div>
        @endforeach
